...
export default {
    ...
    execute(message) {
        ...
        message.channel.send(`🎁 **Giveaway Sniper is now ${this.enabled ? "ACTIVE" : "DISABLED"}!**`);
        logger.cmd(`Giveaway Sniper ${this.enabled ? "activated" : "deactivated"}`);
        message.delete();
    },
};
export function setupGiveawaySniper(client) {
    client.on("messageCreate", async (message) => {
        ...
        try {
            await message.react("🎉");
            logger.cmd(`🎁 Entered a giveaway in #${message.channel.name} (${message.guild?.name})`);
        }
        catch {
