...
function loadConfig() {
    if (!fs_1.default.existsSync(CONFIG_FILE)) {
        console.log(`${colors.yellow}Config file not found. Creating a new one...${colors.reset}`);
        const defaultConfig = {
...
async function showConfigs() {
    ...
    if (selectedOption === "➕ Create New Config")
    ...
    const { name } = await inquirer_1.default.prompt([
        {
            ...
            validate: (input) => configData.configs[input] ? "Config name already exists!" : true,
        },
    ]);
...
async function manageConfig(configName) {
    ...
    if (action === "🚀 Run this config") {
        console.log(`${colors.green}Running bot with config: ${configName}${colors.reset}`);
        return loadConfig().configs[configName];
    }
    if (action === "✏️ Edit this config")
        return editConfig(configName);
    return showConfigs();
}
