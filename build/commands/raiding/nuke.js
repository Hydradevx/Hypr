  ...
  async execute(message) {
    ...
    try {
        ...
        try {
          ...
          for (const msg of fetchedMessages.values()) {
            await msg
              .delete()
              .catch((err) =>
                logger_1.default.error(`Failed to delete message: ${err}`),
              );
          }
        } catch (err) {
          logger_1.default.error(`Error deleting messages: ${err.message}`);
          await channel.send(
              ...
              await member
                ...
                .catch((err) =>
                  logger_1.default.error(
                    `Could not kick ${member.user.tag}: ${err}`,
                  ),
                );
            }
          }
        } catch (err) {
          logger_1.default.error(`Error kicking members: ${err.message}`);
          await channel.send(
      ...
      if (err instanceof Error && err.message === "time") {
        ...
        logger_1.default.error(`Error nuking channel: ${err.message}`);
        await channel.send(
